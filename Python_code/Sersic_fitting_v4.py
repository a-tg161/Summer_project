...
(10) *** In-memory caches don't survive a process restart ***
    Everything in note (6) -- `_RENDERER_CACHE`, `_KERNEL_CACHE`,
    `_ENGINE_CACHE`, and jax's own default (in-memory) compilation cache --
    is a plain Python object that is empty again the moment the interpreter
    exits. That's invisible inside one long-lived call to `everything()`,
    but if galaxies are actually being run as separate process invocations
    (a cluster job array, one `python Sersic_fitting_v3.py` per object,
    resuming after a crash with `SKIP_EXISTING`, etc.) every single process
    re-pays the full ~200-450 s compile cost from note (6) -- which alone
    would make "1 galaxy x 3 filters" look like it takes an hour-plus even
    though the actual sampling is a few minutes. Fixed by pointing jax at a
    persistent, on-disk compilation cache (`jax_compilation_cache_dir`) near
    the top of the file: the first process ever to compile a given (image
    shape, sky_type, sampling settings) kernel writes it there, and every
    later process -- for any galaxy, filter or survey -- loads the compiled
    executable from disk in a couple of seconds instead of recompiling. This
    is the single thing in this file most likely to explain a 1.5-hour
    galaxy if the fits are not literally all running inside one Python
    process; check that first.

(11) *** A NaN data pixel with a finite rms silently NaN'd the whole fit ***
    `_build_inverse_variance()` only zeroed `ivar` for masked, non-finite,
    or non-positive-*rms* pixels -- it never looked at the science image
    itself. `run_fit()` happened to paper over this downstream by replacing
    non-finite pixels in `fit_image` with 0.0, but
    `run_fit_sersic_pointsource()` did not do this at all, so any cutout
    with a NaN science pixel that had a normal rms value fed a NaN straight
    into `resid = data - model_img`. `NaN * 0.0` is `NaN`, so that one pixel
    NaN'd `numpyro.factor`'s *scalar* summed log-density -- for the entire
    100x100 image, every leapfrog step. That is a plausible independent
    contributor to the "badly-mixing posterior" in the convergence warning
    below (a NaN log-density looks like an automatic divergence/rejection,
    not a crash), and is fixed by having `_build_inverse_variance()`
    optionally exclude non-finite *image* pixels too, and by having
    `run_fit_sersic_pointsource()` pass a NaN-safe `data` array the same way
    `run_fit()` already does. `run_fit()` itself is unaffected in output but
    now excludes those pixels from the fit statistics properly instead of
    silently zero-filling them while still calling them "good".

(12) The custom Sersic+point-source fit did not save a .asdf result file at
    all, unlike every natively-supported profile via `_save_result_light()`.
    `_save_result_light()` is now a thin wrapper around a new
    `_write_asdf_result()` that takes the underlying arrays/strings directly
    instead of a `PySersicResults` object, so `run_fit_sersic_pointsource()`
    can write the same tree (input data, loss/renderer/method/prior
    descriptions, posterior-median model image and parameters, full
    posterior) to `{filt}_{profile_key}_data.asdf` the same way `run_fit()`
    does.
...
# Persist compiled XLA executables to disk. note (6) in the module docstring
# amortises compilation *within* one call to everything() via in-memory
# caches (_RENDERER_CACHE / _KERNEL_CACHE / _ENGINE_CACHE) -- but those are
# plain Python dicts, so they are empty again the moment the interpreter
# restarts. If galaxies are being run as separate process invocations (a
# cluster job array, one `python Sersic_fitting_v3.py` per object, restarting
# after a crash, etc.) every single one pays the full ~200-450 s NUTS/SVI
# compile cost from note (6) again, which alone is enough to turn a ~10-15
# minute fit into something that looks like 1.5 hours. This makes that cost
# a one-time-ever cost instead: the first process to compile a given
# (image shape, sky_type, sampling settings) kernel writes it here, and every
# later process -- for any galaxy, any filter, any survey -- loads it from
# disk in a couple of seconds instead of recompiling. Safe to leave on
# unconditionally: a cache hit is just a fast file read, and a cache miss
# behaves exactly as before.
_JAX_CACHE_DIR = os.environ.get(
    "SERSIC_FITTING_JAX_CACHE_DIR",
    os.path.expanduser("~/.cache/jax_sersic_fitting"),
)
...
def _build_inverse_variance(rms, mask, image=None):
    """
    Pre-compute the masked inverse-variance map used by the likelihood -- see
    note (2). Masked, zero/negative-rms and non-finite-rms pixels get
    ivar = 0, which both removes them from the fit and stops a single NaN
    pixel from poisoning the whole log-density (v2 would return NaN in that
    case).

    If `image` is given, pixels where the *science* image itself is
    non-finite are folded into the same ivar = 0 treatment. This matters
    because a NaN data pixel with a perfectly finite rms is otherwise scored
    as "good": `resid * resid * ivar` becomes `NaN * <weight>`, which is NaN
    regardless of the weight, and `numpyro.factor` just adds that NaN into
    the *scalar* log-density for the whole image -- one bad pixel silently
    zeroes out the gradient signal from all 10,000 others for that fit. That
    shows up as a badly-mixing chain (or NaN loss / init failures), not as a
    crash, so it is easy to miss.
    """
    rms = np.asarray(rms, dtype=np.float64)
    good = (~np.asarray(mask, dtype=bool)) & np.isfinite(rms) & (rms > 0)
    if image is not None:
        good &= np.isfinite(np.asarray(image, dtype=np.float64))
...
def _save_result_light(results, fname, model_image, median_params):
    ...
    _write_asdf_result(
        fname,
        image=results.data, rms=results.rms, psf=results.psf,
        mask=results.mask,
        loss_func_str=str(results.loss_func),
        renderer_str=str(results.renderer),
        method_used=results.runtype,
        prior_info=results.prior.__str__(),
        model_image=model_image,
        median_params=median_params,
        posterior_dict=results.idata.to_dict()["posterior"],
    )
...
def run_fit(
    ...
    _, good = _build_inverse_variance(rms, mask, image=image)
    fit_mask = ~good
    n_dropped = int(np.sum(fit_mask) - np.sum(np.asarray(mask, dtype=bool)))
    if n_dropped > 0:
        print(f"[{tag}] [warning] {n_dropped} unmasked pixel(s) had a "
              f"non-finite or non-positive rms, or a non-finite science "
              f"value; masking them out of the fit.")
...
def run_fit_sersic_pointsource(
    ...
    ivar, good = _build_inverse_variance(rms, mask, image=image)
    n_dropped = int(np.sum((~np.asarray(mask)) & (~good)))
    if n_dropped:
        print(f"[{tag}] [warning] {n_dropped} unmasked pixel(s) had a "
              f"non-finite or non-positive rms, or a non-finite science "
              f"value, and were excluded from the fit.")
    # Non-finite pixels are already excluded via ivar = 0 above, but they
    # still flow through `resid = data - model_img` before being multiplied
    # by that zero, and NaN * 0.0 == NaN. Replace them in `data` too so the
    # per-pixel residual is finite everywhere and only the ivar weighting
    # decides what counts -- otherwise a single NaN pixel silently NaNs the
    # summed log-density for the whole 100x100 image every single leapfrog
    # step, which looks like (and causes) a badly-mixing, slow-to-run chain
    # rather than a clean crash.
    fit_image = np.where(np.isfinite(np.asarray(image)), np.asarray(image), 0.0)

    model_kwargs = dict(
        data=jnp.asarray(fit_image),
